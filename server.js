...
var argv = minimist(process.argv.slice(2), {
  default: {
      as_uri: "https://192.168.100.13:8443/",
            ...
            pipeline.create('WebRtcEndpoint', function(error, callerWebRtcEndpoint) {
                ...
                pipeline.create('WebRtcEndpoint', function(error, calleeWebRtcEndpoint) {
                    ...
                    pipeline.create("Composite",function(error,composite){
                        ...
                        composite.createHubPort(function(error,hubPortFromCaller){
                            if(error){
                                pipeline.release();
                                return callback(error)
                            }
                            composite.createHubPort(function(error,hubPortFromCallee){
                                if(error){
                                    pipeline.release();
                                    return callback(error)
                                }
                                composite.createHubPort(function(error,hubPortToOutPut){
                                    if(error){
                                        pipeline.release();
                                        return callback(error)
                                    }
                                    callerWebRtcEndpoint.connect(hubPortFromCaller,function(error){
                                        if(error){
                                            pipeline.release();
                                            return callback(error);
                                        }
                                        calleeWebRtcEndpoint.connect(hubPortFromCallee,function(error){
                                            if(error){
                                                pipeline.release();
                                                return callback(error);
                                            }
                                            hubPortToOutPut.connect(callerWebRtcEndpoint, function(error) {
                                                ...
                                                pipeline.create('RecorderEndpoint', {uri: argv.file_uri}, function(error, recorderEndpoint){
                                                    ...
                                                    hubPortToOutPut.connect(recorderEndpoint,function(error){
                                                        if (error) {
                                                            pipeline.release();
                                                            return callback(error);
                                                        }
